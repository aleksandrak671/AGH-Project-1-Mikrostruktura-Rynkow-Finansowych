...
4. **Wartość obrotu (Kapitał):**
 ...
 - Mimo niższej ceny akcji, Spółka H przyciąga znacznie większy kapitał. Średnia wartość pojedynczej transakcji wynosi dla niej aż ~17 340 PLN, podczas gdy dla droższej Spółki D jest to tylko ~10 572 PLN.
 
 - Również średni dzienny obrót jest gigantyczny w przypadku Spółki H (~27.8 mln PLN) w porównaniu do Spółki D (~8.8 mln PLN), co ostatecznie potwierdza jej dominującą płynność.

**Podsumowanie:** Potwierdziłyśmy założenia. Spółka D to "drogi i powolny" walor o mniejszym przepływie kapitału, a Spółka H to walor "tani, płynny i o wysokim obrocie".
...
**Interpretacja:** jednoznacznie uzasadnia wybór modelu.

 - Najniższa wartość AIC (69899.88) występuje dla rzędu P=1 z uwzględnieniem dni tygodnia (TRUE).
 
 - Modele z dniami tygodnia są systematycznie lepsze od tych bez nich, co potwierdza istotny wpływ dnia na zmienność.
 
 - Zwiększanie liczby fal (P>1) pogarsza wynik (wzrost AIC), co zgodnie z zasadą parsymonii wskazuje, że prostszy model (P=1) jest optymalny.
...
Nasze wyniki potwierdzają, że model jest dobrze dopasowany:
 ...
 - sin1: Parametr jest istotny statystycznie (***), co jest dowodem na to, że wykryłyśmy cykliczny wzorzec ("falę") w danych.

 - day_of_week: Istotność tych zmiennych potwierdza, że dni tygodnia różnią się od siebie, co uzasadnia użycie osobnych parametrów dla każdego dnia.
