...
oblicz_statystyki <- function(df) {
  ...
  wyniki <- data.frame(
    ...
    Srednia_Wartosc_Transakcji = mean(df$Cena * df$Wolumen),
...
1. **Cena a Wolumen:**
 ...
 - Spółka D jest droga (ok. 462 zł), więc inwestorzy kupują ją rzadko i w małych ilościach (mediana wolumenu to zaledwie 13 sztuk).
 
 - Spółka H jest tania (ok. 7.51 zł), więc handluje się nią hurtowo (mediana wynosi aż 1000 sztuk).

2. **Czas oczekiwania (Duracja):**

 - Na transakcję Spółką D trzeba czekać średnio ok. 365 sekund (ponad 6 minut). To oznacza stosunkowo niską płynność.
 
 - Na transakcję Spółką H czeka się znacznie krócej, bo ok. 181 sekund (3 minuty). To dowód na wyższą płynność.

3. **Liczba transakcji:**

 - Spółka H jest znacznie bardziej aktywna w skali całej sesji giełdowej (wraz z dogrywką) -> ma w ciągu dnia niemal dwa razy więcej transakcji (ok. 1604) niż Spółka D (ok. 834).
 
4. **Wartość obrotu (Kapitał):**

 - Mimo znacznie niższej ceny pojedynczej akcji, Spółka H przyciąga dużo większy kapitał. Średnia wartość pojedynczej transakcji wynosi dla niej ok. 17 341 PLN, podczas gdy dla bardzo drogiej Spółki D jest to zaledwie ok. 10 573 PLN.
 
 - Podobnie wygląda średni obrót dzienny: w przypadku Spółki H jest to gigantyczna kwota ok. 27,8 mln PLN, w porównaniu do ok. 8,8 mln PLN dla Spółki D, co ostatecznie potwierdza dominującą płynność Spółki H.

**Podsumowanie:** Potwierdziłyśmy początkowe założenia. Spółka D to walor "drogi i powolny" o relatywnie mniejszym przepływie kapitału, a Spółka H to walor "tani, bardzo płynny i o wysokim obrocie".


## Agregacja do 1 sekundy

Będziemy musiały "zbić" (zagregować) nasze dane do częstotliwości 1-sekundowej. Nie bierzemy ostatniej ceny z danej sekundy (Robimy to, aby wynik był wiarygodny: w ciągu jednej sekundy może wystąpić wiele transakcji, a ostatnia z nich może być przypadkowa (np. o bardzo małym wolumenie i nietypowej cenie). Średnia ważona sprawia, że cena dla danej sekundy odzwierciedla to, po ile faktycznie wymieniono większość akcji, eliminując wpływ pojedynczych, przypadkowych transakcji ("szum informacyjny"). Liczymy średnią ważoną wolumenem (VWAP). 

**Wzór:**

$$\text{VWAP} = \frac{\sum (\text{Cena} \times \text{Wolumen})}{\sum \text{Wolumen}}$$
...
1. **Ekstremalna Kurtoza ("Grube ogony"):**
 ...
 - Dla obu spółek kurtoza jest bardzo duża (ok. **77.5** dla Spółki D i **11.6** dla Spółki H), znacznie powyżej wartości 3 (charakterystycznej dla rozkładu normalnego). Czyli rozkład stóp zwrotu nie jest normalny. Na rynku często zdarzają się nagłe, gwałtowne skoki cen.

2. **Ujemna Autokorelacja (Efekt "odbicia"):**

 - Obie spółki mają ujemną autokorelację pierwszego rzędu (odpowiednio ok. **-0.20** i **-0.32**). Potwierdza to występowanie zjawiska odbicia bid-ask (bid-ask bounce). Cena w krótkim terminie "skacze" góra-dół pomiędzy ofertami kupna i sprzedaży, zamiast podążać w jednym, płynnym trendzie.

**Podsumowanie:** nasze otrzymane wyniki (wysoka kurtoza i ujemna autokorelacja) potwierdzają specyficzne własności danych śróddziennych, odróżniające je od klasycznych danych dziennych.
...
agreguj_5min <- function(df) { 
  df %>%
    ...
    # wszystko co dzieje się po notowaniach ciągłych 
    # (czyli wyliczone czasy > 16:50) traktujemy jako JEDEN wspólny przedział "17:05".
    mutate(interval_5min = if_else(
      hour(interval_5min) == 17 | (hour(interval_5min) == 16 & minute(interval_5min) > 50),
      ymd_hms(paste(date(interval_5min), "17:05:00")),
      interval_5min
    )) %>%

    # grupujemy po spółce, dacie i nowym 5-minutowym interwale
...
dzienne_D <- dane_5min_D %>%
  ...
  summarise(
    # bierzemy cenę z pierwszego słupka (np. 09:05) by mieć jakiś punkt startowy
    Open_Day = first(Close), 
    
    # bierzemy cenę z ostatniego słupka (16:50) jako koniec dnia
...
# Najpierw liczymy średnią dla każdej pory dnia (np. dla wszystkich 10:00)
srednie_sloty <- dane_5min_D %>%
  group_by(slot_czasowy) %>%
  summarise(mean_ret = mean(zwrot_log, na.rm = TRUE), .groups = "drop")

# 3. szykujemy tabelę którą wrzucimy do modelu (regresji)
dane_fff <- dane_5min_D %>%
  # doklejamy policzone wyżej średnie
  left_join(srednie_sloty, by = "slot_czasowy") %>%
  mutate(
    # odejmujemy średnią ze stóp zwrotu (odchylenia od średniej)
    zwrot_demeaned = zwrot_log - mean_ret,
    
    # wartości dokładnie = 0 zmieniamy na NA (by uniknąć błędu log(0))
    zwrot_demeaned = if_else(zwrot_demeaned == 0, NA_real_, zwrot_demeaned)
  ) %>%
  group_by(date) %>%
  # numerujemy słupki w ciągu dnia: 1, 2, 3... 
  # by model FFF wiedział który to jest moment dnia (czy rano czy wieczór)
  mutate(n = row_number()) %>% 
  ungroup() %>%
  mutate(
    # Liczymy y_fff na podstawie poprawnych (odśrednionych) zwrotów
    y_fff = 2 * log(abs(zwrot_demeaned)) - 2 * log(sqrt(sigma_norm_sq)),
...
# tabela z wynikami AIC jako dowód
kable(wyniki_aic, caption = "Wybór rzędu opóźnienia P (Kryterium AIC)")
...
Powyższa tabela jednoznacznie uzasadnia wybór modelu.

 - Najniższa wartość AIC (**46951.67**) występuje dla rzędu **P=3** z uwzględnieniem dni tygodnia (TRUE).
 
 - Modele z uwzględnieniem dni tygodnia charakteryzują się niższymi wartościami AIC niż modele bez tej zmiennej, co wskazuje, że włączenie efektu tygodniowego poprawia dopasowanie modelu.
 
 - Zwiększanie liczby fal poprawia wynik (spadek AIC) do poziomu P = 3. Dalsze zwiększanie rzędu (P > 3) prowadzi do wzrostu AIC, co zgodnie z zasadą parsymonii, wskazuje, że model z trzema harmonicznymi jest optymalny.
...
# sprawdzenie istotności parametrów
print("Podsumowanie modelu FFF:")
summary(model_fff)
...
# dzielimy odśredniony zwrot przez wyliczoną sezonowość
# np: jeśli o 9:00 rano zmienność jest zwykle 2 razy większa (sezonowość = 2), to dzielimy zwrot przez 2
# dzięki temu zostanie nam "czysty" zwrot, pozbawiony deterministycznego efektu pory dnia.
dane_fff_final$zwrot_odsezonowany <- dane_fff_final$zwrot_demeaned / dane_fff_final$sezonowosc
```

Nasze wyniki potwierdzają, że model jest dobrze dopasowany:

 - **`sin1` oraz `sin2`**: Parametry te są bardzo silnie istotne statystycznie (odpowiednio z poziomami istotności `***` oraz `**`), co jest ostatecznym dowodem na to, że rozszerzenie modelu do P=3 było słuszne, a model poprawnie wyłapał złożony, cykliczny wzorzec ("fale") w danych śróddziennych.
 - **Zmienna `day_of_week`**: Analiza podsumowania modelu wykazała, że statystycznie istotny (na poziomie 5%, `*`) okazał się tylko jeden parametr odpowiadający za dni tygodnia, konkretnie `day_of_week.L` (oznaczający trend liniowy). W praktyce oznacza to, że bazowy wzorzec sezonowości (kształt "U") jest uniwersalny dla wszystkich dni, ale w przekroju całego tygodnia występuje pewna systematyczna tendencja (np. stopniowy, liniowy wzrost lub spadek ogólnej "nerwowości" rynku od poniedziałku do piątku), co w pełni uzasadnia pozostawienie tej zmiennej w modelu.
 
Generujemy nasze wykresy: porównanie średnich modułów przed i po (przed silna krzywa "U", teraz wygładzona zmienność), autokorelacja przed i po, oraz sam wyizolowany kształt sezonowości.
...
Wykres "przed" wykazuje silny, deterministyczny kształt litery U (bardzo wysokie wartości na otwarciu i pod koniec sesji). Po zastosowaniu poprawnego modelu FFF (zoptymalizowanego do rzędu P=3 i pracującego na odśrednionych stopach zwrotu), zmienność w ciągu dnia uległa wygładzeniu. Na czerwonej krzywej widoczne są już tylko losowe, stochastyczne wahania, a wyraźny trend systematycznego wzrostu zmienności na krańcach sesji notowań ciągłych został pomyślnie zneutralizowany.
...
# PRZED: wartości bezwzględne odśrednionych zwrotów
acf(scale(abs(dane_fff_final$zwrot_demeaned)),
    lag.max = 500,
    ylim = c(0, 0.2),
    main = "ACF |r| - Przed")

# PO: wartości bezwzględne zwrotów odsezonowanych
acf(scale(abs(dane_fff_final$zwrot_odsezonowany)),
    lag.max = 500,
    ylim = c(0, 0.2),
    main = "ACF |r| - Po (FFF)")
...
Na wykresie ACF dla wartości bezwzględnych stóp zwrotu przed modyfikacją widoczna jest wyraźna struktura cykliczna. lokalne maksima pojawiają się w przybliżeniu co jeden dzień sesyjny (około 95 lagów), co wskazuje na obecność śróddziennej sezonowości zmienności.

Po zastosowaniu metody FFF amplituda tych cyklicznych wahań uległa częściowej redukcji. Regularna struktura związana z porą dnia jest słabsza, jednak nie została całkowicie wyeliminowana.

Utrzymująca się dodatnia autokorelacja dla małych opóźnień oraz częściowo widoczne fale wynikają z naturalnego klastrowania zmienności (volatility clustering), które ma charakter stochastyczny i nie stanowi elementu deterministycznej sezonowości śróddziennej.

Wyniki wskazują zatem na ograniczenie, lecz nie całkowite usunięcie sezonowości zmienności.
...
Wykres przedstawia wyizolowaną składową sezonowości śróddziennej. Zmienność jest najwyższa na otwarciu sesji, następnie szybko spada i osiąga minimum w środkowej części dnia, po czym ponownie rośnie przed zamknięciem. Wzorzec ten odpowiada klasycznej sezonowości typu "U", choć nie jest idealnie symetryczny.
...
Do modelowania zmienności dziennej zastosowano model klasy ARMA-GARCH. Jako specyfikację przyjęto wariant ARMA(0,0)-GARCH(1,1).

Wstępna analiza autokorelacji dziennych stóp zwrotu nie wykazała istotnych zależności liniowych, co uzasadnia przyjęcie modelu ARMA(0,0). Zastosowanie wyższych rzędów prowadziłoby do nadmiernego dopasowania bez istotnej poprawy jakości modelu.

Człon GARCH(1,1) stanowi standardową i empirycznie dobrze uzasadnioną specyfikację opisującą zjawisko klastrowania zmienności.
...
spec <- ugarchspec(
  ...
  # ZMIANA: Zmieniamy arma na (0,0) dla dziennych stóp zwrotu
...
porownanie_H <- dane_es %>%
  ...
  # NORMALIZACJA – żeby lepiej zobaczyć kształt sezonowości (średnia = 1)
  mutate(
    Przed = Przed / mean(Przed),
    Po = Po / mean(Po),
    # zamieniamy tekst "09:05" na datę by wykres wiedział, że to oś czasu
    czas = as.POSIXct(paste("2024-01-01", slot_czasowy), format="%Y-%m-%d %H:%M")
  )

# p1: wykres "przed" (górny)
p1 <- ggplot(porownanie_H, aes(x = czas, y = Przed)) + 
  geom_line(color = "blue") +
  labs(title = "SPOLKA_H: Przed (z 'U')", y = "Znormalizowane |r|") + 
  theme_minimal() + 
  scale_x_datetime(date_labels="%H:%M")

# p2: wykres "po" (dolny)
# rysujemy dane oczyszczone; oś y jako |epsilon| (szum/reszta)
p2 <- ggplot(porownanie_H, aes(x = czas, y = Po)) + 
  geom_line(color = "red") +
  labs(title = "SPOLKA_H: Po (Engle i Sokalska)", y = "Znormalizowane |epsilon|") + 
  theme_minimal() + 
  scale_x_datetime(date_labels="%H:%M")
...
Na górnym wykresie widoczna jest wyraźna sezonowość śróddzienna.
Zmienność jest najwyższa na otwarciu sesji, następnie spada w godzinach południowych i częściowo rośnie pod koniec dnia. Wzorzec ten ma charakter zbliżony do klasycznego efektu „U”, choć nie jest idealnie symetryczny.
...
Celem naszej analizy było usunięcie deterministycznej sezonowości śróddziennej (kształtu "U”) ze stóp zwrotu.
 ...
 - **Dla Spółki D (metoda FFF):** Nasz model FFF (zoptymalizowany do rzędu P=3), pracujący na poprawionych odśrednionych stopach zwrotu, skutecznie zniwelował ekstremalną cykliczność zmienności na otwarciu i zamknięciu notowań ciągłych.
 
 - **Dla Spółki H (metoda Engle-Sokalskiej):** Procedura odsezonowania, przy użyciu adekwatnego modelu ARMA(0,0)-GARCH(1,1), wyeliminowała cykliczne wzorce widoczne na wykresach autokorelacji (ACF).

**Wniosek:** Uzyskane i oczyszczone szeregi czasowe są wolne od deterministycznego wpływu pory dnia i reprezentują teraz czystą zmienność stochastyczną.

# Podsumowanie końcowe

Nasza analiza potwierdziła występowanie typowych cech mikrostruktury rynku w ujęciu intraday: grubych ogonów rozkładu, ujemnej autokorelacji stóp zwrotu oraz silnej sezonowości zmienności w kształcie litery "U". Wykorzystanie poprawnego matematycznie modelu Elastycznej Formy Fouriera oraz metody Engle-Sokalskiej zoptymalizowanej dla danych dziennych, pozwoliło usunąć cykliczne, godzinowe wahania. Wyeliminowanie tego zjawiska (tzw. wygładzenie pików na wykresach ACF) sprawiło, że dane nadają się do budowy docelowych modeli ACD w późniejszym etapie.